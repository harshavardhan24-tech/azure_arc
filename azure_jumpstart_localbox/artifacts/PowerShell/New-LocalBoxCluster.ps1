...
azcopy cp 'https://azlocalvhds.blob.core.windows.net/images/AzLocal2603.vhdx' "$($LocalBoxConfig.Paths.VHDDir)\AzL-node.vhdx" --recursive=true --check-length=false --log-level=ERROR
azcopy cp 'https://azlocalvhds.blob.core.windows.net/images/AzLocal2603.sha256' "$($LocalBoxConfig.Paths.VHDDir)\AzL-node.sha256" --recursive=true --check-length=false --log-level=ERROR
